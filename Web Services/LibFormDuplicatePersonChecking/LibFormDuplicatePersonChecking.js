...
module.exports.main = async function (ffCollection, vvClient, response) {
    ...
    let jaroMinScore = .65;                         //This is the minimum score on the JaroWinkler area that should consider a match.  Using this because it picks up stragglers that the overall score does not.
    ...
    try {
        ...
        var CalculateMatchScore = function (passedWord, compareString) {
            ...
            if (compareString.length > 2) {
                ...
                if (damerauResult != null) {
                    if (damerauResult == 0) {  //equals exact match.
                        score++;
                    }
                }
                ...
                if (diceResult != null) {
                    if (diceResult == 1) {  //equals exact match.
                        score++;
                    }
                }
                ...
                if (jaccardResult != null) {
                    if (jaccardResult == 1) {  //equals exact match.
                        score++;
                    }
                }
                ...
                if (jarowinkResult != null) {
                    if (jarowinkResult == 1) {  //equals exact match.
        ...
        if (!personVal) {  //|| !personVal.value
            ...
            if (typeof PersonObj != 'undefined') {  //PersonObj object found
                //Oject found.  Now test key properties that they are present.
                if (!PersonObj.hasOwnProperty('firstname')) {  //Verify if property is present.
                    errors.push('firstname property is not available in the PersonObject.');
                }
                else if ((PersonObj.firstname.trim()).length == 0) {  //Verify if a value was entered.
                    errors.push('firstname property needs a value in the PersonObject.');
                }

                if (!PersonObj.hasOwnProperty('lastname')) {  //Verify if property is present.
                    errors.push('lastname property is not available in the PersonObject.');
                }
                else if ((PersonObj.lastname.trim()).length == 0) {  //Verify if a value was entered.
                ...
                if (!PersonObj.hasOwnProperty('otheridquery')) {  //Verify if property is present.
                    errors.push('otheridquery property is not available in the PersonObject.');
                }
                else if ((PersonObj.otheridquery.trim()).length == 0) {  //Verify if a value was entered.
        ...
        if (searchQueryResult.meta.status !== 200) { throw new Error(`There was an error when calling getIndRec. ${errorMessageGuidance}`) }
        if (searchQueryData === null) { throw new Error(`Data was not able to be returned when calling getIndRec. ${errorMessageGuidance}`) }
        ...
        if (secondSearchResult.meta.status !== 200) { throw new Error(`There was an error when calling getIndRec. ${errorMessageGuidance}`) }
        if (secondSearchData === null) { throw new Error(`Data was not able to be returned when calling getIndRec. ${errorMessageGuidance}`) }
