...
module.exports.main = function (ffCollection, vvClient, response) {
    /*Script Name:   UserMgmtEnable
     Customer:      VV Code Example.
     Purpose:       The purpose of this script is to enable a user. 
    ...
    logger.info('Start of the process UserMgmtEnable at ' + Date());
            ...
            return vvClient.scripts.runWebService('LibUserUpdate', updateUserObject).then(function (userResp) {
                ...
                if (userResp.meta.status === 200) {
                    ...
                    if (userResp.data[0] == 'Success') {
                        ...
                        throw new Error('The call to enable the user returned with an error. ' + userResp.data[1]);
                    }
                    else {
                        throw new Error("The call to enable the user returned with an unhandled error. Please try again or contact a system administrator.");
                    }
                }
                else {
