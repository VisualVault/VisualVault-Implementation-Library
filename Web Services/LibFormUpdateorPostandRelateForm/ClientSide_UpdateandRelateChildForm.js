//Update and Relate One Child Form

var UpdateandRelateChild = function () {
    ...
    var requestObject = $.ajax({
        type: "POST",
        url: VV.BaseAppUrl + 'api/v1/' + VV.CustomerAlias + '/' + VV.CustomerDatabaseAlias + '/scripts?name=ParentFormUpdateandRelateChildForm',
    ...
    if (typeof (resp.status) != 'undefined') {
        ...
        if (typeof (resp.data[0]) != 'undefined') {
            if (resp.data[0] == 'Success') {
                messageData = 'Child form was updated and related successfully.';
                ...
                messageData = "The call to udpate the child form returned with a successful status, but unexpected results. The status code returned was: " + resp.data[0];
            }
        }
        else if (typeof (resp.data[1]) != 'undefined') {
            messageData = "The call to udpate the child form returned with a successful status, but unexpected results. The status message returned was: " + resp.data[1];
        }
        else {
            messageData = 'The call to update the child form returned with a successful status, but with invalid data. There is no status code or status message to display.';
        }
    }
    else {
