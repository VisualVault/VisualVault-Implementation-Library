//Post and Relate New Child Form

var PostandRelateChild = function () {
    ...
    var requestObject = $.ajax({
        type: "POST",
        url: VV.BaseAppUrl + 'api/v1/' + VV.CustomerAlias + '/' + VV.CustomerDatabaseAlias + '/scripts?name=ParentFormPostandRelateNewChildForm',
    ...
    if (typeof (resp.status) != 'undefined') {
        ...
        if (typeof (resp.data[0]) != 'undefined') {
            if (resp.data[0] == 'Success') {
                messageData = "Child form was created, posted, and related successfully.";
                ...
                messageData = "The call to post the child form returned with a successful status, but unexpected results. The status code returned was: " + resp.data[0];
            }
        }
        else if (typeof (resp.data[1]) != 'undefined') {
            messageData = "The call to post the child form returned with a successful status, but unexpected results. The status message returned was: " + resp.data[1];
        }
        else {
            messageData = 'The call to post the child form returned with a successful status, but with invalid data. There is no status code or status message to display.';
        }
    }
    else {
